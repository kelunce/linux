...
static program_t *epilog PROT((void)) {
	...
	if (i != A_LINENUMBERS && i != A_FILE_INFO && 
	    ...
	    i != A_ARGUMENT_INDEX)
	    size += __align(mem_block[i].current_size);
    ...
    size += __align((num_func * sizeof(function_t)));   /* A_FUNCTIONS */

    /* function flags */
    size += __align(((comp_last_inherited + num_func) * sizeof(unsigned short)));

    /* A_ARGUMENT_INDEX */
    if (mem_block[A_ARGUMENT_INDEX].current_size)
	size += __align(num_func * sizeof(unsigned short)); 
    ...
    *prog = NULL_program;
    ...
    p += __align(sizeof(function_t) * num_func);
    ...
    p += __align(sizeof(unsigned short) * (comp_last_inherited + num_func));
	...
	while (i--)
	    dest[i] = *((unsigned short *) mem_block[A_ARGUMENT_INDEX].block
			+ func_index_map[i]);
	p += __align(num_func * sizeof(unsigned short));
    } else {
